...
def med_detail(request, med_uid):
    ...
    Note:
        여기서 발생하는 400,401error는 try문으로 발생하는 error가 아니라 editMedicine을 통해 발생한 error값이 return 되어 발생하는 것이다.
    """
    #사용자 아이디 확인
...
def saltSave(salt_arr, medAdd):
    ...
    medicine의 salt목록 insert, update 함수
        ...
        Raises:
            400: salt insert, update 실패할 시,(salt에 데이터에 유효하지 않은 데이터가 있을경우 발생) 400 error return한다.
